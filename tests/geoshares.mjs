...
// trebled — Goldland came out 0.1% of the map on the founding seed of "The Silver
...
// Goldland — the northern rival — actually borders Silver.
const base = new URL('../js/', import.meta.url).href;
const G = await import(base + 'geo.js');
const ok = (l, c, x = '') => console.log((c ? 'PASS ' : 'FAIL ') + l + (x ? ' | ' + x : ''));

// Classify a coarse grid once (0 sea, 1 gold, 2 silver, 3 electrum, 4 unassigned),
...
function survey(nation, salt) {
  ...
  for (let r = 0; r < rows; r++) for (let col = 0; col < cols; col++) {
    ...
    if (G.inPoly(p, g.ring)) {
      code = G.inPoly(p, g.halves.goldland) ? 1
        : G.inPoly(p, g.halves.silver) ? 2
          : G.inPoly(p, g.halves.electrum) ? 3 : 4;
      land++;
    }
    grid[r * cols + col] = code;
    c[code]++;
  }
  // Border cells: a Goldland cell with a Silver cell 4-connected to it.
  let goldSilver = 0;
  ...
  return { gold: pct(1), silver: pct(2), electrum: pct(3), none: c[4], goldSilver };
}
...
const cases = [
  ...
  // Names that used to rail the solver at the trebled amplitude (Goldland ate the
  // map, Electrum vanished), plus a couple of ordinary ones.
  ['Republic', 0], ['Concordia', 0], ['Goldland', 0], ['Silver', 0], ['Freeland', 0],
];
...
for (const [n, s] of cases) {
  ...
  if (r.none > 0) anyNone++;
  if (r.gold < 5 || r.silver < 5 || r.electrum < 5) anyMissing++;
}
...
ok('Goldland is never a sliver', worstGold >= 20, `worst goldland ${worstGold.toFixed(1)}%`);
ok('Goldland always borders Silver', worstBorder >= 20, `worst gold-silver border ${worstBorder} cells`);

// The founding map specifically — the one the bug was reported against.
const tsr = survey('The Silver Republic', 0);
ok('The Silver Republic founds with a real Goldland', tsr.gold >= 20 && tsr.goldSilver >= 20,
  `goldland ${tsr.gold.toFixed(1)}%, border ${tsr.goldSilver} cells`);
...
ok('a typical seed calibrates near the Goldland target', typ.gold >= 38 && typ.gold <= 50,
  `goldland ${typ.gold.toFixed(1)}%`);
