...
// Bombing: a raid wears the enemy's will down and knocks their army back.
{
  ...
  w.military.wars.push({ id: 'w', foreign: 'goldland', started: 0, front: 0, exhaustion: 0.5, allies: [] });
  const r = DEP.bomb(w, pid, 'goldland');
  ...
  ok('a second raid at once is refused', !DEP.bomb(w, pid, 'goldland').ok);
}
...
// You cannot bomb without an air force, or where there is no war.
{
  ...
  f.atWar = true;
  w.military.wars.push({ id: 'w', foreign: 'goldland', started: 0, front: 0, exhaustion: 0, allies: [] });
  ok('no wings, no raid', !DEP.bomb(w, pid, 'goldland').ok);
  w.military.airforce = 2;
  const elec = w.foreign.find((x) => x.id === 'electrum');
